...
(define-test (:index :basic)
  (let* ((idx (make-index (list #'rec-foo #'rec-bar)))
    ...
    (assert (eq rec1 (index-find idx (index-key idx rec1) nil)))
    (assert (eq rec2 (index-find idx (index-key idx rec2) nil)))
  ...
  (let ((idx (make-index (list #'rec-foo) :uniq? t)))
    (with-index
      (let* ((rec (index-add idx (make-rec :foo 1 :baz "ab")))
        ...
        (assert (null (index-find idx key nil)))
        (assert (index-add idx rec))
        (index-commit)
        (assert (eq rec (index-find idx key nil)))
